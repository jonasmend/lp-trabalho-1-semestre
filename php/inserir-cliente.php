<?php
  ...
  if(!empty($nome) && !empty($cpf) && !empty($cep) && !empty($data_nascimento) && !empty($rua) && !empty($numero) && !empty($bairro)){
    ...
    $sql = "INSERT INTO cliente (nome, cpf, data_nascimento, cep, rua, bairro, numero, complemento) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    $query = $pdo->prepare($sql);
    $query->execute(array($nome, $cpf, $data_nascimento, $cep, $rua, $bairro, $numero, $complemento));
    Conexao::desconectar();
  }
